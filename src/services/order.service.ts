...
import { AuthenticatedRequestBody, IUser, OrderT, ProcessingOrderT } from '../interfaces';
...
export const clearAllOrdersService = async (
  ...
  try {
    ...
    const dropCompleteCollection = await Order.deleteMany({ 'user.email': req.user?.username });
